 ...
 * enable this code later on, added for completion for now 
  ara::core::Result<ara::core::InstanceSpecifier> is_kvs_result = ara::core::InstanceSpecifier::MakeInstanceSpecifier(
...
void PersistentMemAccessor::StoreVariantCodingData(uint32_t data_to_store, const std::string& key_to_store) {
  // try {
  //   (*key_value_storage)->SetValue<std::uint32_t>(ara::core::StringView(key_to_store), data_to_store);
  ...
  //     logger_ctx.LogInfo() << "VcPersistentMemAccessor : #INFO: The read value from: "_sv << key_to_store << " is: "_sv
  //                           << restored_value;
  //   } else {
...
ReadConfigureSarTriggerEvents0136VcEventData() {
   ...
   * interface in ECU extract. This code shall be updated once persistency interface has been added
  */
...
activateSarStorage0131VcEventDataType PersistentMemAccessor::
ReadActivateSarStorage0131VcEventData() {
