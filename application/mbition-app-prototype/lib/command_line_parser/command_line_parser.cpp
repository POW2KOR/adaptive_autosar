...
: parent_{ parent }, arg_index_{ start_index }, str_pos_{ kFirstCharacterIndex },
  cur_opt_{ kInvalidOption }, cur_argument_{ nullptr } {
    ParseNextOption();
}
...
void CommandLineParser::Iterator::ParseNextOption() noexcept {
    ...
    if (arg_index_ == kInvalidIteratorIndex) {
        ...
        StopParsing();
    } else if (str_pos_ == kFirstCharacterIndex &&
               parent_.argv_[arg_index_][kFirstCharacterIndex] != '-') {
        ...
        if (opt == nullptr) {
            ...
            if (opt[kSecondCharacterIndex] == ':') {
                ...
                /* #00 If current parameter string ends with '\0' the argument
                 * is passed as a separate argument (e.g. "-a Value"). */
                if (*cur_argument_ == '\0') {
...
const char* CommandLineParser::Iterator::GetOptionArgument() const noexcept {
    return cur_argument_;
}
...
void CommandLineParser::ValidateOptionList() noexcept {
    if (options_list_ == nullptr) {
        ...
        auto isAlphaChar = [](char c) {
            return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z');
        };
        auto prev = [this](const char* ptr) {
            return (ptr != this->options_list_ ? *(ptr - 1) : '\0');
        };
        ...
        for (; *ptr != '\0'; ptr++) {
            if (*ptr == ':') {
                if (!isAlphaChar(prev(ptr))) {
                    assert(false && "Argument specifier ':' must follow an "
                                    "alphabetic character.");
                    break;
                }
            } else if (!isAlphaChar(*ptr)) {
                assert(false && "Option list must only contain alphabetic "
                                "characters or ':' to specify an argument.");
                break;
            }
        }
