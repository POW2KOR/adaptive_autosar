...
void ActivationManagerBase::initializeDesiredWakeup() {
  if (0 == desired_wakeup_.tv_sec && 0 == desired_wakeup_.tv_nsec) {
    /* get the current time of day and use it for later sleeps. All sleeps will use this time as base and only add
     * (multiple) cycle times */
    clock_gettime(CLOCK_MONOTONIC, &desired_wakeup_);
    desired_wakeup_.tv_nsec += cycle_time_.count();
    desired_wakeup_.tv_sec  += desired_wakeup_.tv_nsec / std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(1)).count();
    desired_wakeup_.tv_nsec %= std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(1)).count();
  }
}
...
void ActivationManagerBase::calculateJitter() {
  ...
  if (actual_wakeup_.tv_sec == desired_wakeup_.tv_sec) {
    ...
    if (actual_wakeup_.tv_sec == desired_wakeup_.tv_sec + 1) {
      jitter_time_nsec_ = actual_wakeup_.tv_nsec + std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(1)).count() - desired_wakeup_.tv_nsec;
    }
    else {
      if (actual_wakeup_.tv_sec == desired_wakeup_.tv_sec - 1) {
        jitter_time_nsec_ = actual_wakeup_.tv_nsec - std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(1)).count() - desired_wakeup_.tv_nsec;
        ...
        logger_->critical("expected: {:}.{:06d}, actual: {:}.{:06d}.",
                          desired_wakeup_.tv_sec, desired_wakeup_.tv_nsec,
                          actual_wakeup_.tv_sec,  actual_wakeup_.tv_nsec);
#endif /* SPDLOG */
        exit(1);
      }
    }
...
#ifdef SPDLOG
    logger_->critical("missed {:d} wakeups, now total missed {:12d} wakeups (too late by {} ns)",
                      missed_, total_missed_, jitter_time_nsec_);
#endif /* SPDLOG */
  }

  /* print warning if we did not miss a wakeup but we have only left less than 90% of the cycle time */
  if ((0 == missed_) &&
      (0 < jitter_time_nsec_) &&
      (static_cast<int64_t>(jitter_time_nsec_) > (cycle_time_.count() / 10))) {
#ifdef SPDLOG
    logger_->warn("when activated only less than 90 % of cycle time is left (cycle: {} µs, delay: {} µs)",
                  std::chrono::duration_cast<std::chrono::microseconds>(cycle_time_).count(), jitter_time_nsec_ / 1000);
#endif /* SPDLOG */
  }
