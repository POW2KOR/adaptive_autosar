 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  Entry point of the dummy 2 application.
 *
 *      \details  -
 *
 *********************************************************************************************************************/
...
int main()
{
    ...
    if (!init_result.HasValue()) {
        ...
        application::DummySsaServer ssaServer;
        return_value = ssaServer.Run();

        if (EXIT_FAILURE == return_value) {
            char const* msg{"failed to run dummy SSA server for offering services."};
            ara::core::Abort(msg);
        }
